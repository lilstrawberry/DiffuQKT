...
def info_nce_loss(a, b, temperature=0.07):
    a = F.normalize(a, p=2, dim=-1)  # [batch_size, d]
    b = F.normalize(b, p=2, dim=-1)  # [batch_size, d]

    similarity_matrix = torch.matmul(a, b.T)  # [batch_size, batch_size]
...
class DiffuQKT(nn.Module):
    ...
    def forward(self, last_pro, last_skill, last_ans, next_pro, next_skill, next_ans, mask, is_train=True):
        ...
        if is_train:
            loss += info_nce_loss(h_state[q_mask], h_origin_state[q_mask], 0.3) * self.lamda_2
